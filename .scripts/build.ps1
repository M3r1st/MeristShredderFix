...
$builder = [BuildProject]::new("MeristShredderFix", $srcDirectory, $sdkPath, $gamePath)
...
if ($null -ne (Get-Command "X2ProjectGenerator.exe" -ErrorAction SilentlyContinue))
{
    ...
    &"X2ProjectGenerator.exe" "$srcDirectory\MeristShredderFix" "--exclude-contents" "--verify-only"
